...
| `diagnose_save_error` | **Diagnostic**: recover the REAL error behind a failed grid save. When a save fails with only the generic "record raised at least one error" (the classic SOAP plane truncates the reason; the modern plane never serializes a hidden tab's grid errors at all), this replays the failing change over the screen's **classic ASPX WebForms callback protocol** — a 5th plane, diagnostic-only — and returns the screen's actual alert text plus per-row/cell errors (e.g. PY309000's cross-row "Percent should be 100 for sum of all banks", AR301000's "The value must be less than or equal to 100", invisible to both API planes). Page path auto-resolves from the site map. Cell keys must be the CLASSIC grid's column names, not the modern plane's — an unknown key is REFUSED with the real column list + a suggestion rather than sent (e.g. `CreditAmt` → `CuryCreditAmt`). CAVEAT: it POSTS a real Save, so a change that is actually *valid* persists — but `possibly_saved: true` is NOT a confirmation, it's a "no error came back" signal that can also mean a silent no-op (e.g. the cell is read-only for that row, which the tool checks and names when detectable); the result's `note` always says to verify via run_dac_odata. Requires allow_write. |
